...
struct _AutoState {
  float Kp=0.55f, Ki=0.28f, Kd=0.10f;    
  ...
  const float MAX_ASCENT_THR=0.55f;     
  const float MAX_DESCENT_THR=0.52f;
  const float ALT_ALPHA=0.18f;          
  const float RAMP_UP_RATE=0.10f;       
  const float RAMP_DOWN_RATE=0.35f;
  const float SP_SLEW_RATE=0.18f;       
